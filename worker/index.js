...
async function loadState(env) {
  ...
  const [threadRows, commentRows] = await env.DB.batch([
    ...
    env.DB.prepare("SELECT thread_id, payload FROM comments ORDER BY created_at DESC"),
  ]);
...
function decorateComment(comment) {
  ...
  return { ...comment, stance: positions.some((item) => item.id === comment.stance) ? comment.stance : "unsure", claimType: claims.some((item) => item.id === comment.claimType) ? comment.claimType : "opinion", impactAreas: validImpacts(comment.impactAreas), sourceUrl: safeUrl(comment.sourceUrl), helpfulness: helpfulness.map((item) => ({ ...item, count: signalCounts[item.id] })), pendingReportsCount: 0 };
}

function decorateThread(thread, state) {
  const comments = (state.comments[thread.id] || []).map(decorateComment);
...
function homePayload(state) {
  ...
  return { rooms: roomPayloads, featured: { dailyIssues: [...sorted].sort((a, b) => b.value - a.value || b.heat - a.heat).slice(0, 6), hotThreads: sorted.slice(0, 8), groundedThreads: [...sorted].sort((a, b) => b.evidenceRate - a.evidenceRate || b.value - a.value).slice(0, 6), bridgingThreads: [...sorted].sort((a, b) => b.bridgeSignals - a.bridgeSignals || b.value - a.value).slice(0, 6), newestThreads: sorted.slice(0, 8), megathreads: sorted.filter((item) => item.megathread).slice(0, 6), politicians: board.politicians.sort((a, b) => b.activityCount - a.activityCount).slice(0, 6) }, moderation: board.moderation, meta: board.meta };
}
...
async function api(request, env, pathname) {
  ...
  if (request.method === "POST" && pathname === "/api/threads") {
    ...
    const room = clean(body.room), title = clean(body.title), summary = clean(body.summary), text = clean(body.body), sourceUrl = safeUrl(body.sourceUrl), decisionPrompt = clean(body.decisionPrompt), impactAreas = validImpacts(body.impactAreas), tags = [...new Set((Array.isArray(body.tags) ? body.tags : String(body.tags || "").split(/[,、]/)).map(clean).filter(Boolean))].slice(0, 4);
    ...
    if (!rooms.some((item) => item.id === room) || title.length < 8 || summary.length < 20 || text.length < 30 || !sourceUrl || decisionPrompt.length < 10 || !impactAreas.length || !targets.some((item) => item.id === targetType) || (!targetId && targetLabel.length < 2)) return json({ error: !sourceUrl ? "invalid_source_url" : "invalid_thread_target" }, 400);
    const now = new Date().toISOString(); const thread = { id: id(), room, author: clean(body.author).slice(0, 32) || "名無しさん", title: title.slice(0, 120), summary: summary.slice(0, 240), body: text.slice(0, 4000), sourceUrl, sourceKind: ["official", "news", "analysis", "other"].includes(body.sourceKind) ? body.sourceKind : "other", decisionPrompt: decisionPrompt.slice(0, 180), impactAreas, tags, targetType, targetId: targetId || id(), targetLabel, reactions: counts(reactions), positions: counts(positions), createdAt: now, updatedAt: now, megathread: Boolean(body.megathread), pinned: Boolean(body.megathread), locked: false, slowModeSeconds: body.megathread ? 60 : 0 };
  ...
  if (request.method === "POST" && commentMatch) { const thread = state.threads.find((item) => item.id === commentMatch[1]); if (!thread) return json({ error: "thread_not_found" }, 404); if (await limited(env, request, `comment:${thread.id}`, Math.max(15000, Number(thread.slowModeSeconds || 0) * 1000))) return json({ error: "comment_cooldown" }, 429); const text = clean(body.body), sourceUrl = safeUrl(body.sourceUrl), impactAreas = validImpacts(body.impactAreas); if (text.length < 2 || !positions.some((item) => item.id === body.stance) || !claims.some((item) => item.id === body.claimType) || !impactAreas.length || (body.claimType === "fact" && !sourceUrl)) return json({ error: body.claimType === "fact" && !sourceUrl ? "fact_requires_source" : "invalid_comment" }, 400); const now = new Date().toISOString(); const comment = { id: id(), author: clean(body.author).slice(0, 32) || "名無しさん", body: text.slice(0, 1000), stance: body.stance, claimType: body.claimType, impactAreas, sourceUrl, helpfulness: counts(helpfulness), createdAt: now }; await env.DB.prepare("INSERT INTO comments (id, thread_id, payload, created_at) VALUES (?, ?, ?, ?)").bind(comment.id, thread.id, JSON.stringify(comment), now).run(); state.comments[thread.id].unshift(comment); return json({ ok: true, thread: decorateThread(thread, state), comments: state.comments[thread.id].map(decorateComment) }, 201); }
